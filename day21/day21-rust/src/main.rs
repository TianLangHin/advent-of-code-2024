...
fn part1(codes: &Vec<&str>) {
    ...
    for code in codes {
        ...
        let mut second_robot = Vec::new();
        for (code, _) in first_robot {
            ...
            second_robot.extend(possible_solutions.into_iter());
        }

        let min_length = second_robot.iter().map(|(s, _)| s.len()).min().unwrap();
        second_robot.retain(|(s, _)| s.len() == min_length);

        let mut third_robot = Vec::new();
        for (code, _) in second_robot {
            let mut possible_solutions = VecDeque::from([("".to_string(), 0)]);
            let steps = once('A').chain(code.chars()).zip(code.chars()).enumerate();
            for (i, (start, end)) in steps {
                let (x_start, y_start) = *dir_keypad.get(&start).unwrap();
            ...
            third_robot.extend(possible_solutions.into_iter());
        }

        let min_length = third_robot.iter().map(|(s, _)| s.len()).min().unwrap();
